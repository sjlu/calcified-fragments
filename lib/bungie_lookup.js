...
var handleError = function (response, body) {
  // bungie timed out, throw a recoverable error
  // to see if we can try again
  if (response.statusCode === 504) {
    throw new errors.RecoverableError("Bungie timed out, try again in a little bit!")
  }

  if (body.ErrorCode === 5) {
    throw new errors.RecoverableError("Bungie's API is undergoing maintenance")
  } else if (body.ErrorCode == 1665) {
    throw new errors.RecoverableError("Your Bungie data is private, please make it public!")
  } else if (body.ErrorCode == 1670 || body.ErrorCode == 1669) {
  ...
  } else if (body.ErrorCode == 1601) {
    throw new errors.RecoverableError("Unable to find Destiny account information, but the gamertag is valid")
